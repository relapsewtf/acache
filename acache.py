...
class ActivityCacheParser:
    ...
    def parse_json_blobs(self, rows: Iterable[sqlite3.Row]) -> List[Dict[str, Any]]:
        ...
        for row in rows:
            ...
            # parse appid/payload - handle both lowercase and capitalized column names
            appid = rowdict.get('appid') or rowdict.get('AppId')
            if appid:
                try:
                    rowdict['appid_parsed'] = json.loads(appid)
                    logger.debug("Parsed appid: %s -> %s", appid, rowdict['appid_parsed'])
                except json.JSONDecodeError as e:
                    logger.debug("Failed to parse appid: %s (error: %s)", appid, e)
                    rowdict['appid_parsed'] = None
            # Parse payload - handle both lowercase and capitalized column names
            payload = rowdict.get('payload') or rowdict.get('Payload')
            if payload:
                try:
                    rowdict['payload_parsed'] = json.loads(payload)
                except json.JSONDecodeError:
                    rowdict['payload_parsed'] = None
            # times - handle both lowercase and capitalized column names
            starttime_key = 'starttime' if 'starttime' in rowdict else 'StartTime'
            endtime_key = 'endtime' if 'endtime' in rowdict else 'EndTime'
            if starttime_key in rowdict:
                rowdict['starttime_readable'] = self.filetime_to_datetime(rowdict.get(starttime_key))
            if endtime_key in rowdict:
                rowdict['endtime_readable'] = self.filetime_to_datetime(rowdict.get(endtime_key))
    ...
    def analyze_data(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for entry in data:
            appid_parsed = entry.get('appid_parsed')
            app = 'unknown'
            if appid_parsed:
                if isinstance(appid_parsed, list) and len(appid_parsed) > 0:
                    # appid is an array of objects, get application from first object
                    app = appid_parsed[0].get('application', 'unknown')
                elif isinstance(appid_parsed, dict):
                    # appid is a single object
                    app = appid_parsed.get('application', 'unknown')
            if not app:
                app = 'unknown'
            appcounts[app] += 1
